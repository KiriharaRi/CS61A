...
def more_boar(player_score, opponent_score):
    ...
    while player_score > 0 or opponent_score > 0:
        flag = digit(player_score) < digit(opponent_score)
        if flag == False:
            return flag
        player_score //= 10
        opponent_score //= 10
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    max = 1
    turn_grade = make_averaged(roll_dice)
    max_averaged = turn_grade(1,dice)
    i = 1
    while i <= 10:
        data = turn_grade(i,dice)
        if data > max_averaged:
            max_averaged = data
            max = i
        i += 1
    return max
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    boar_score = score + piggy
    if more_boar(boar_score,opponent_score):
        return 0
    elif piggy >= cutoff:
        return 0
    else:
